...
export const ORCHESTRATOR_RUNTIME_PACKAGES = [
  ...
  { name: "@joko/adapter-transcription-scribe", workspacePath: "packages/adapter-transcription-scribe", candidatePath: "node_modules/@joko/adapter-transcription-scribe" },
  { name: "@joko/adapter-transcription-sauc", workspacePath: "packages/adapter-transcription-sauc", candidatePath: "node_modules/@joko/adapter-transcription-sauc" },
  ...
  { name: "@joko/messaging", workspacePath: "packages/messaging", candidatePath: "node_modules/@joko/messaging" },
  ...
  { name: "@joko/tool-document", workspacePath: "packages/tool-document", candidatePath: "node_modules/@joko/tool-document" },
  { name: "@joko/tool-ios-simulator", workspacePath: "packages/tool-ios-simulator", candidatePath: "node_modules/@joko/tool-ios-simulator" },
...
export const ORCHESTRATOR_RUNTIME_CRITICAL_IMPORTS = [
  ...
  "@joko/adapter-transcription-scribe",
  "@joko/adapter-transcription-sauc",
  "@joko/code-host",
  "@joko/contracts",
...
/**
 * Exercises the built Extension Library manager and its node:sqlite Worker
 * under the exact Electron-Node runtime that will host the Orchestrator.
 */
export function extensionLibraryElectronSmokeSource(): string {
  return [
    ...
    'import { pathToFileURL } from "node:url";',
    'if (typeof process.versions.electron !== "string" || process.versions.electron === "") {',
    '  throw new Error("Extension Library runtime smoke must run under Electron-Node.");',
    '}',
    'const runtimeArgument = process.argv[2];',
    'if (typeof runtimeArgument !== "string" || runtimeArgument === "" || !isAbsolute(runtimeArgument) || resolve(runtimeArgument) !== runtimeArgument) {',
    '  throw new Error("Extension Library smoke requires one normalized absolute runtime root.");',
    '}',
    'const runtimeRoot = await canonicalDirectory(runtimeArgument, "runtime root");',
    'const managerEntry = await canonicalRegularFile(runtimeRoot, resolve(runtimeRoot, "dist", "extension-library-manager.js"), "Extension Library manager entry");',
    'const workerEntry = await canonicalRegularFile(runtimeRoot, resolve(runtimeRoot, "dist", "extension-library-sql-worker.js"), "Extension Library SQL Worker entry");',
    'const managerModule = await import(pathToFileURL(managerEntry).href);',
    'if (typeof managerModule.ExtensionLibraryManager !== "function") {',
    '  throw new Error("The built Extension Library manager entry does not expose its manager.");',
    '}',
    'const smokeRoot = await mkdtemp(join(await realpath(tmpdir()), "joko-extension-library-runtime-smoke-"));',
    'const connectionId = "desktop-runtime-smoke";',
    'const authority = {',
    '  extensionId: `extension_${"e".repeat(32)}`,',
    '  extensionRevision: 1n,',
    '  resourceId: "resource-runtime-smoke",',
    '  resourceRevision: 1n,',
    ...
    '  backendGeneration: 1,',
    '  name: "Runtime smoke",',
    '  library: { schemaVersion: 1, extensionEntry: "extensions/runtime-smoke.js" }',
    '};',
    'const manager = new managerModule.ExtensionLibraryManager({',
    '  rootDirectory: join(smokeRoot, "library-owner"),',
    ...
    'let changes;',
    'let selectedValue;',
    'let phase = "manager initialization";',
    'try {',
    '  await manager.initialize();',
    '  phase = "session open";',
    '  const session = await manager.openSession({ authority, connectionId, assertAuthorityCurrent: () => undefined });',
    '  sessionId = session.sessionId;',
    '  phase = "database open";',
    '  const database = await manager.databaseOpen(sessionId, connectionId, { path: "state.sqlite", create: true, readonly: false });',
    '  handleId = database.handleId;',
    '  phase = "database migration";',
    '  userVersion = await manager.databaseMigrate(sessionId, connectionId, handleId, [',
    '    { version: 1, statements: ["CREATE TABLE runtime_smoke(value TEXT NOT NULL)"] }',
    '  ]);',
    '  phase = "database insert";',
    '  const inserted = await manager.databaseExecute(sessionId, connectionId, handleId, "INSERT INTO runtime_smoke(value) VALUES (?)", ["electron-worker"]);',
    '  changes = inserted.changes;',
    '  phase = "database select";',
    '  const selected = await manager.databaseExecute(sessionId, connectionId, handleId, "SELECT value FROM runtime_smoke");',
    '  selectedValue = selected.rows[0]?.value;',
    '  if (userVersion !== 1 || changes !== 1n || selected.rows.length !== 1 || selectedValue !== "electron-worker") {',
    '    throw new Error("The Extension Library SQL Worker returned an invalid round trip.");',
    '  }',
    '  phase = "database close";',
    '  await manager.databaseClose(sessionId, connectionId, handleId);',
    '  handleId = undefined;',
    '  phase = "session close";',
    '  if (!await manager.closeSession(sessionId, connectionId)) {',
    '    throw new Error("The Extension Library runtime smoke session did not close.");',
    '  }',
    '  sessionId = undefined;',
    '} catch (error) {',
    '  throw new Error(`Extension Library runtime smoke failed during ${phase}: ${error instanceof Error ? error.message : String(error)}`, { cause: error });',
    '} finally {',
    '  if (handleId !== undefined && sessionId !== undefined) {',
    '    await manager.databaseClose(sessionId, connectionId, handleId).catch(() => undefined);',
    '  }',
    '  if (sessionId !== undefined) await manager.closeSession(sessionId, connectionId).catch(() => undefined);',
