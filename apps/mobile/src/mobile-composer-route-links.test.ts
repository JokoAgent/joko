...
describe("mobile composer task links", () => {
  it("segments bare and Markdown task links while retaining adjacent text and punctuation", () => {
    const segments = segmentMobileComposerRoutePaste(
      "See https://user:pass@example.test/view?token=secret&theme=dark#/tasks/session%20one?message=message-1&auth=bad, then [Other @ task](#/tasks/session-2)."
    );
  ...
  it("rejects malformed, duplicate, non-task, and non-Joko route identities", () => {
    ...
    expect(parseMobileComposerRouteHref("#/projects/project")).toBeUndefined();
