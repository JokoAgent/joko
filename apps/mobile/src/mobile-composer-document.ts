...
export interface MobileComposerDraft {
  ...
  readonly mentions: readonly MobileComposerSessionMention[];
}
...
export function normalizeMobileComposerDraft(value: MobileComposerDraft): MobileComposerDraft {
  ...
  const mentions = value.mentions.map((candidate) => {
    ...
    if (!Number.isSafeInteger(candidate.start) || !Number.isSafeInteger(candidate.end)
      ...
      || value.text.slice(candidate.start, candidate.end) !== mobileSessionMentionToken(displayText)) {
...
export function cloneMobileComposerDraft(draft: MobileComposerDraft): MobileComposerDraft {
  ...
  return { text: exact.text, mentions: exact.mentions.map((mention) => ({ ...mention })) };
}
  ...
  return first.text === second.text && first.mentions.length === second.mentions.length
    && first.mentions.every((mention, index) => {
      ...
      return candidate !== undefined && mention.kind === candidate.kind && mention.mentionId === candidate.mentionId
        && mention.sessionId === candidate.sessionId && mention.displayText === candidate.displayText
        && mention.start === candidate.start && mention.end === candidate.end;
    });
}
...
export function insertMobileSessionMention(
  ...
  const retainedCount = current.mentions.filter((mention) => mention.end <= range.start || mention.start >= range.end).length;
  if (retainedCount >= maximumSessionMentions) throw new Error("A task message can reference at most 8 other tasks.");
  const token = mobileSessionMentionToken(candidate.displayText);
  const prefix = range.start > 0 && !/\s/u.test(current.text[range.start - 1] ?? "") ? " " : "";
  const suffix = range.end < current.text.length && !/\s/u.test(current.text[range.end] ?? "") ? " " : "";
  const result = replaceMobileComposerRange(current, range, `${prefix}${token}${suffix}`);
  ...
  const mentions = [...result.draft.mentions, mention].sort((left, right) => left.start - right.start || left.end - right.end);
...
export function mobileComposerInput(draft: MobileComposerDraft): InputContent {
  ...
  return create(InputContentSchema, {
    parts: [
      ...
      ...exact.mentions.map((mention) => create(InputPartSchema, {
...
export function mobileInputSummary(input: InputContent | undefined, typedMetadataTrusted = true): string {
  ...
  const mentions = input.parts.flatMap((part) => {
    if (part.content.case === "sessionMention") return [{ label: part.content.value.displayText || part.content.value.sessionId }];
    if (part.content.case === "workspaceMention") return [{ label: part.content.value.displayText || part.content.value.relativePath }];
    if (part.content.case === "resourceMention") return [{ label: part.content.value.displayText || part.content.value.resourceId }];
    if (part.content.case === "artifactMention") return [{ label: part.content.value.displayText || part.content.value.artifactId }];
    return [];
  });
  const rangesValid = validMentionRanges(text, mentions.length, input.mentionRanges, input.pastedTextRanges);
...
function normalizeDisplayText(value: string): string {
  if (typeof value !== "string") throw new Error("The local Joko task reference label is invalid.");
  const exact = value.trim();
  if (!exact || exact.length > maximumDisplayCharacters || /[\u0000-\u001f\u007f]/u.test(exact)) {
    throw new Error("The local Joko task reference label is invalid.");
  }
  return exact;
}
