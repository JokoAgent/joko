...
it("waits for the live appearance acknowledgement, replays current overrides after queued theme changes and retires old connections", async () => {
  ...
  const render = async (methods = api) => act(async () => root!.render(<InteractiveTerminalPanel controller={{ ...methods, state: { connectionState: "connected", activeProfile: { id: "profile", serverId: "server" } } } as unknown as AppController} sessionId="session-one" terminalId="terminal-one" active capabilities={capabilities} t={t} onState={vi.fn()} />));
  await render();
