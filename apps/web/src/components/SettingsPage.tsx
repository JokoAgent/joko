      ...
      <div className="settings-layout">
        ...
        <div className={cx("settings-content-scroll", (section === "providers" || section === "import") && "settings-content-scroll--workbench")} ref={contentRef} key={section}>
          <div className={cx("settings-content", (section === "providers" || section === "import") && "settings-content--workbench")} id={`settings-panel-${section}`} role="tabpanel" aria-labelledby={`settings-tab-${section}`} tabIndex={0}>
            ...
            {section === "shortcuts" && <><AppShortcutsSettings controller={controller} overrides={controller.state.preferences.appShortcutOverrides} t={t} /><GamepadSettings t={t} listSkills={controller.listSkills} serverId={controller.state.activeProfile?.serverId} connected={controller.state.connectionState === "connected"} /></>}
...
function PiSettings({ controller, snapshot, runAction, t }: { readonly controller: AppController; readonly snapshot: AppSnapshot; readonly runAction: RunAction; readonly t: Translator }): JSX.Element {
  ...
  const resourceBackendAvailable = snapshot.backends.some((backend) => resourceKindsForBackend(backend).length > 0);
  ...
  })}</div>}<header className="settings-section-heading settings-section-heading--actions"><h3>{t("settings.managedResources")}</h3><Button tone="primary" disabled={!resourceBackendAvailable} title={!resourceBackendAvailable ? t("settings.resourceBackendUnavailable") : undefined} onClick={() => setResourceEditorOpen(true)}>{t("settings.addResource")}</Button></header>{!resourceBackendAvailable && <p className="muted settings-unavailable-hint" role="status">{t("settings.resourceBackendUnavailable")}</p>}<PiPackagesSection controller={controller} backends={snapshot.backends} resources={snapshot.resources} runAction={runAction} t={t} /><ResourceEditor open={resourceEditorOpen} snapshot={snapshot} t={t} onClose={() => setResourceEditorOpen(false)} onSave={(draft) => { setResourceEditorOpen(false); runAction("add-resource", () => controller.addResource(draft)); }} /></>;
}
...
function ResourceEditor({ open, snapshot, t, onClose, onSave }: { readonly open: boolean; readonly snapshot: AppSnapshot; readonly t: Translator; readonly onClose: () => void; readonly onSave: (draft: ResourceDraft) => void }): JSX.Element {
  const capableBackends = snapshot.backends.filter((backend) => resourceKindsForBackend(backend).length > 0);
  const initialBackendId = capableBackends[0]?.id ?? "";
  const initialKind = resourceKindsForBackend(capableBackends[0])[0] ?? "skill";
  const [draft, setDraft] = useState<ResourceDraft>(() => emptyResourceDraft(initialBackendId, initialKind));
  useEffect(() => {
    if (open) setDraft(emptyResourceDraft(initialBackendId, initialKind));
  }, [open, initialBackendId, initialKind]);
  const selectedBackend = capableBackends.find((backend) => backend.id === draft.backendId);
  const resourceKinds = resourceKindsForBackend(selectedBackend);
  const valid = resourceKinds.includes(draft.kind) && resourceDraftIsValid(draft);
  const setResourceBackend = (backendId: string): void => {
    const backend = capableBackends.find((candidate) => candidate.id === backendId);
    const kinds = resourceKindsForBackend(backend);
    setDraft((current) => ({
      ...current,
      backendId,
      kind: kinds.includes(current.kind) ? current.kind : kinds[0] ?? "skill",
      ...(kinds.includes(current.kind) || current.source.kind === "local"
        ? {}
        : { source: emptyResourceAcquisitionDraft("local") })
    }));
  };
