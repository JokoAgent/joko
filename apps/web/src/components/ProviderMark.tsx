...
export function ProviderMark({ providerId, name, size = 18 }: {
  readonly providerId: string;
  readonly name: string;
  readonly size?: number;
}): JSX.Element {
  const normalizedId = providerId.trim().toLocaleLowerCase();
  const normalizedName = name.trim().toLocaleLowerCase();
  if (normalizedId === "openai" || normalizedName === "openai") return <OpenAiMark size={size} />;
  if (normalizedId === "anthropic" || normalizedName === "anthropic") return <AnthropicMark size={size} />;
  return <>{providerMonogram(name)}</>;
}

function OpenAiMark({ size }: { readonly size: number }): JSX.Element {
...
function AnthropicMark({ size }: { readonly size: number }): JSX.Element {
  return <svg width={size} height={size} viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    <path fill="currentColor" d="M13.827 3.52h3.603L24 20h-3.603l-6.57-16.48zm-7.258 0h3.767L16.906 20h-3.674l-1.343-3.461H5.017l-1.344 3.461H0L6.57 3.52zm4.132 9.876L8.453 7.247 6.205 13.396z" />
  </svg>;
}

function providerMonogram(name: string): string {
  const normalized = name.trim();
  return normalized.length > 0 ? normalized[0]!.toLocaleUpperCase() : "P";
}
