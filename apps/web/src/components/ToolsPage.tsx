...
export function BrowserCanvas({ browser, page, allowUploads, screenshotBlobId, targets, locale, t, controller, runAction, onCapture, onStoreCapture, onUpload, onAction }: { readonly browser: BrowserView; readonly page?: BrowserPageView; readonly allowUploads: boolean; readonly screenshotBlobId?: string; readonly targets: readonly BrowserCommentDraftTarget[]; readonly locale: string; readonly t: Translator; readonly controller: AppController; readonly runAction: RunAction; readonly onCapture: (browserId: string, pageId: string) => void; readonly onStoreCapture: (browserId: string, pageId: string, blobId: string) => void; readonly onUpload: (browserId: string, pageId: string, file: File) => void; readonly onAction: (browserId: string, pageId: string, action: BrowserTakeoverActionView) => Promise<void> }): JSX.Element {
  ...
  useEffect(() => {
    ...
    return registerWorkspaceHtmlPreviewSurface(browser.id, page.id, rootRef.current);
  }, [browser.id, page?.id]);
...
function BrowserLedger({ controller, browser, page, locale, t, runAction }: { readonly controller: AppController; readonly browser: BrowserView; readonly page: BrowserPageView; readonly locale: string; readonly t: Translator; readonly runAction: RunAction }): JSX.Element {
  ...
  return <section className="browser-ledger" aria-label={t("browser.durableActivity")}>
    ...
    <div className="browser-ledger__columns">
      <section><h4>{t("browser.transfers")}</h4>{transfers.length === 0 ? <p className="muted">{loading ? t("common.loading") : t("browser.noTransfers")}</p> : <div className="browser-ledger__list">{transfers.map((transfer) => <article key={transfer.id}><span className="browser-ledger__icon">{transfer.direction === "upload" ? <Upload aria-hidden="true" /> : <Download aria-hidden="true" />}</span><span><strong>{transfer.fileName}</strong><small>{t(`browser.direction.${transfer.direction}`)} · {formatBytes(transfer.byteSize)} · {formatRelativeTime(transfer.completedAt ?? transfer.startedAt, locale)}</small>{transfer.error !== undefined && <small className="danger-text">{transfer.error}</small>}</span><Pill tone={transfer.state === "completed" ? "success" : transfer.state === "failed" ? "danger" : transfer.state === "running" ? "accent" : "neutral"}>{transfer.state}</Pill>{transfer.direction === "download" && transfer.state === "completed" && transfer.blobId !== undefined && <ArtifactDownloadButton tone="ghost" ownerKey={JSON.stringify([browser.id, page.id, transfer.id, transfer.blobId])} connectionOwner={controller.downloadArtifact} label={t("common.download")} errorLabel={t("workspace.downloadUnavailable")} action={(context) => controller.downloadArtifact(transfer.blobId as string, transfer.fileName, context)} />}</article>)}</div>}</section>
...
function ExtensionTools({ controller, snapshot, runtimeSessionId, selectedId, locale, t, runAction, onSelect }: {
  ...
  const openPackage = (extension: ExtensionCatalogEntryView): void => {
    const ownerResourceId = extension.owner.kind === "resource" ? extension.owner.resourceId : undefined;
    const ownerBackendId = ownerResourceId === undefined
      ? undefined
      : snapshot.resources.find((resource) => resource.id === ownerResourceId)?.backendId;
    const sessionBackendId = runtimeSessionId === undefined
      ? undefined
  ...
  return <div className={cx("extension-browser", selectedId !== undefined && "has-selection")}>
    <section className="extension-browser__catalog" aria-label={t("extensions.catalog") }>
      <div className="extension-browser__controls">
        ...
        </div>{hasPackageUpdates && <Button tone="ghost" onClick={() => setPackageBatchOpen(true)}><PackageCheck aria-hidden="true" />{t("extensions.package.updateAll")}</Button>}<Button tone="ghost" onClick={() => setSourcesOpen(true)}><FolderPlus aria-hidden="true" />{t("extensions.sources.manage")}</Button>
        </div>
      </div>
    ...
    <ExtensionPackageRemovalDialog
      ...
      t={t}
      runAction={runAction}
      onClose={() => setPackageRemoval(undefined)}
      onChanged={async () => {
        onSelect(undefined);
        setRefreshRevision((value) => value + 1);
      }}
    ...
    <ExtensionPackageBatchDialog
      controller={controller}
      resources={snapshot.resources}
      open={packageBatchOpen}
      t={t}
      runAction={runAction}
      onClose={() => setPackageBatchOpen(false)}
      onChanged={async () => {
        const current = selectedRef.current;
        if (current !== undefined) await refreshSelected(current).catch(() => undefined);
        else setRefreshRevision((value) => value + 1);
      }}
    />
