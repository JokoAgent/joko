...
describe("TerminalProvider", () => {
  ...
  test("publishes delayed exit confirmation after a failed remote stream and refuses restart until then", async () => {
    ...
    try {
      ...
      expect((await stream.next()).value).toMatchObject({ kind: "reset", terminal: { exitConfirmed: false } });
      const lateState = stream.next();
      const settled = vi.fn();
      void lateState.then(settled, settled);
      ...
      expect((await lateState).value).toMatchObject({ kind: "state", terminal: { status: "failed", exitConfirmed: true } });
