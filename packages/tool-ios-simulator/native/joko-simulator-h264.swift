...
let productH264Requested = CommandLine.arguments.contains(
    "--enable-h264-stream"
) || CommandLine.arguments.contains("--stream-h264")
let productHIDRequested = CommandLine.arguments.contains(
    "--enable-continuous-input"
)

@_silgen_name("joko_simulator_kit_unmasked_surface")
...
func configurePrivateMetalShaderCache() {
    ...
    guard let configuredCacheDirectory = environment[
        "JOKO_IOS_SIDECAR_METAL_CACHE_DIR"
    ] else { return }
...
guard CommandLine.arguments.contains("--stdio") ||
      CommandLine.arguments.contains("--probe") ||
      CommandLine.arguments.contains("--stream-h264") else {
    fail("a supported mode is required")
}
...
if CommandLine.arguments.contains("--probe") {
    exit(nativeProbe.framebufferCapture ? 0 : 1)
}

if CommandLine.arguments.contains("--stream-h264") {
    guard productH264Enabled,
          let screen = retainedFramebufferScreen,
          let screenID = retainedFramebufferScreenID,
          let framesPerSecond = Int(argument("--fps")),
          (1...60).contains(framesPerSecond),
          let scalingPercent = Int(argument("--scale")),
          (1...100).contains(scalingPercent),
          let orientation = CommandLine.arguments.firstIndex(of: "--orientation")
            .flatMap({ index in CommandLine.arguments.indices.contains(index + 1)
              ? CommandLine.arguments[index + 1] : nil }),
          orientation == "PORTRAIT" || orientation == "LANDSCAPE" else {
        fail("native H.264 stream is unavailable")
    }
    ...
    do {
        while cancellationRequested == 0 {
            let (delta, overflow) = UInt64(sequence).multipliedReportingOverflow(by: interval)
            if overflow { break }
            let (target, targetOverflow) = startedAt.addingReportingOverflow(delta)
            if targetOverflow { break }
            while cancellationRequested == 0 && DispatchTime.now().uptimeNanoseconds < target {
                Thread.sleep(forTimeInterval: 0.005)
            }
            if cancellationRequested != 0 { break }
            let captured = try captureCurrentFramebuffer(
            ...
            guard let encodedMetadata = try? JSONSerialization.data(withJSONObject: metadata),
                  encodedMetadata.count > 0,
                  encodedMetadata.count <= 512,
                  accessUnit.bytes.count <= maxFramebufferBytes else {
                fail("native H.264 output is invalid")
            }
            let bodyLength = 2 + encodedMetadata.count + accessUnit.bytes.count
            guard bodyLength <= 16 * 1024 * 1024 else {
                fail("native H.264 output exceeds limit")
            }
            var output = Data()
            output.reserveCapacity(4 + bodyLength)
            let length = UInt32(bodyLength)
            output.append(UInt8(length & 0xff))
            output.append(UInt8((length >> 8) & 0xff))
            output.append(UInt8((length >> 16) & 0xff))
            output.append(UInt8((length >> 24) & 0xff))
            let metadataLength = UInt16(encodedMetadata.count)
            output.append(UInt8(metadataLength & 0xff))
            output.append(UInt8((metadataLength >> 8) & 0xff))
            output.append(encodedMetadata)
            output.append(accessUnit.bytes)
            write(output)
        }
    } catch {
        encoder.invalidate()
        fail("native H.264 capture or encoding failed")
    }
    encoder.invalidate()
    exit(0)
}
