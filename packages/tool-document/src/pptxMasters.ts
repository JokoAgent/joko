/**
 * joko-document/pptxMasters.ts —— pptxgenjs 母版版式(纯几何 + 色板,不捆图不捆字体)。
 *
 * 六套版式:
...
export const PPTX_LAYOUT_IDS = {
  cover: 'JOKO_COVER',
  section: 'JOKO_SECTION',
  content: 'JOKO_CONTENT',
  comparison: 'JOKO_COMPARISON',
  metrics: 'JOKO_METRICS',
  image: 'JOKO_IMAGE',
} as const;
...
export interface DefineJokoPptxMastersOptions {
  theme: DocsTheme;
  footer: boolean;
  footerLabel?: string;
}
 ...
 */
export function defineJokoPptxMasters(
  pptx: InstanceType<(typeof import('pptxgenjs'))['default']>,
  options: DefineJokoPptxMastersOptions,
): void {
